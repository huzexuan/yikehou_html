...
<template>
	<view id="loginBox">
		<view class="form_box">
			...
			<view class="inp_box">
				<input type="text" class="inp" v-model="school_name" autocomplete="off" />
				<view class="icon">
	...
	export default {
		...
		methods: {
			init() {
				...
				if (user) {
					if (user.schoolName && user.userPsw) {
						this.school_name = user.schoolName
						this.password = user.schoolPsw
						this.checked = true
					}
				}
			},
			...
			up_pas() {
				uni.navigateTo({
					url: "/pages/login/password?type=1"
				})
			},
			async submit() {
				let _res = await API.postJson('schoolLogin', {
